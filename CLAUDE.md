...
`compete` is itself a **Claude Code Skill** (`skills/compete/SKILL.md` at the root), not a
...
test — it checks output against `skills/compete/schemas/`. Aim for 0 schema errors. Regenerate the
...
spec is `skills/compete/references/data-schema.md`; the schemas are in `skills/compete/schemas/` (JSON Schema Draft
...
| --- | --- | --- | --- |
| `analyze_repo.py` | Product Intelligence | `product.json` | `skills/compete/references/product-intelligence.md` |
| `discover_competitors.py` | Discovery (`plan`/`build`) | `competitors.json` | `skills/compete/references/competitor-discovery.md` |
| `collect_intelligence.py` | Collection (`plan`/`build`) | 6 per-competitor datasets | `skills/compete/references/intelligence-dimensions.md` |
| `build_report.py` | Graph + Visualization | `report.json`, `report.html` | `skills/compete/references/data-schema.md` |
...
the result into `skills/compete/templates/report.html` at the `__INSIGHTKIT_DATA__` placeholder to
...
- **Schema, reference doc, and code change together.** If you add/change a field,
  update the relevant `skills/compete/schemas/*.schema.json`, bump `schema_version` (minor =
  additive, major = breaking), document it in `skills/compete/references/data-schema.md`, and run the
  matching `--validate`.
